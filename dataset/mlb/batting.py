...
def get_matchup_stats(batter_id, pitcher_id, season, feature_names=MATCHUP_STATS):
    ...
    versus_stats = [0 for _ in feature_names]
...
def get_batter_season_stats(player_id, game_date, year, feature_names=BATTER_STATS):
    ...
    for val in feature_names:
        ...
        if season_val in ZERO_VALUES:
            season_val = 0.0
        results.append(float(season_val))

    return results


def get_batter_last_x_days_stats(
    player_id, game_date, year, last_x, feature_names=BATTER_STATS
):
    t_minus_1_game_date = datetime.strptime(game_date, "%m/%d/%Y") - timedelta(days=1)
