...
class FangraphsLineupAverageDataset(Dataset):
    def __init__(self):
        super().__init__()
        self.csv_file_prefix = "./csv_data/fangraphs_advanced_lineup_average"
        self.batter_days = 5
    ...
    def get_batter_features(self, game_date, team_id, team_lineup):
        ...
        team_two_year_stats = get_two_year_stats(game_date, team_id)

        for player_id in team_lineup:
            player_season = get_player_stats(player_id, team_batter_season)
            lineup_season.append(player_season)
        ...
        lineup_last_two_array = np.array(lineup_last_two).mean(axis=0).tolist()

        features.extend(lineup_season_array)
        features.extend(lineup_last_x_array)
        features.extend(lineup_last_two_array)
        return features
